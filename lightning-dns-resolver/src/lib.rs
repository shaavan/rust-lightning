...
mod test {
	...
	impl MessageRouter for DirectlyConnectedRouter {
		...
		fn create_blinded_paths<T: secp256k1::Signing + secp256k1::Verification>(
			...
			Ok(vec![BlindedMessagePath::one_hop(recipient, local_node_receive_key, context, &keys, secp_ctx).unwrap()])
		}
	}
	...
	async fn resolution_test() {
		...
		let reply_path =
			BlindedMessagePath::one_hop(payer_id, receive_key, query_context, &*payer_keys, &secp_ctx).unwrap();
