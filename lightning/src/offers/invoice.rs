...
impl TryFrom<PartialInvoiceTlvStream> for InvoiceContents {
	...
	fn try_from(tlv_stream: PartialInvoiceTlvStream) -> Result<Self, Self::Error> {
		...
		if offer_tlv_stream.issuer_id.is_none() && offer_tlv_stream.paths.is_none() {
			...
			if let Some(requested_amount_msats) = invoice_request.inner.amount_msats() {
...
mod tests {
	...
	#[test]
	fn parses_invoice_with_wrong_amount_when_request_omits_amount() {
		let expanded_key = ExpandedKey::new([42; 32]);
		let entropy = FixedEntropy {};
		let nonce = Nonce::from_entropy_source(&entropy);
		let secp_ctx = Secp256k1::new();
		...
		let parsed_invoice = Bolt12Invoice::try_from(buffer).unwrap();
		assert_eq!(parsed_invoice.amount_msats(), 2000);
	}
