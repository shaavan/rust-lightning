...
impl Bolt12Invoice {
	pub(crate) fn is_no_ir_amount_and_offer_in_currency(&self) -> bool {
		matches!(
			self.contents,
			InvoiceContents::ForOffer { ref invoice_request, .. }
				if invoice_request.amount_msats().is_none()
					&& invoice_request.inner.offer.is_amount_currency()
		)
	}
}
