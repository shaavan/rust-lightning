...
macro_rules! invoice_derived_signing_pubkey_builder_methods {
	($self: ident, $self_type: ty) => {
		...
		pub(super) fn for_offer_using_keys(
			invoice_request: &'a InvoiceRequest, custom_amount_msat: Option<u64>, payment_paths: Vec<BlindedPaymentPath>,
...
mod tests {
	...
	fn parses_invoice_with_fallback_address() {
		...
		#[cfg(not(c_bindings))]
		let invoice_builder =
			invoice_request.respond_with_no_std(None, payment_paths(), payment_hash(), now()).unwrap();
		#[cfg(c_bindings)]
