...
macro_rules! invoice_builder_methods {
	...
	$self: ident, $self_type: ty, $return_type: ty, $return_value: expr, $type_param: ty $(, $self_mut: tt)?
			...
			CC::Target: CurrencyConversion,
		{
			...
			let minimum_offer_msats = match invoice_request.resolve_offer_amount(currency_conversion)? {
...
mod tests {
	...
	fn invoice_offer_id_matches_offer_id() {
		...
		let offer =
			OfferBuilder::new(recipient_pubkey()).amount_msats(1000).unwrap().build().unwrap();
	...
	fn verifies_invoice_signature_with_tagged_hash() {
		...
		let offer = OfferBuilder::new(node_id).amount_msats(1000).unwrap().build().unwrap();
