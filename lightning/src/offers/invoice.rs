...
impl TryFrom<PartialInvoiceTlvStream> for InvoiceContents {
	...
	fn try_from(tlv_stream: PartialInvoiceTlvStream) -> Result<Self, Self::Error> {
		...
		if offer_tlv_stream.issuer_id.is_none() && offer_tlv_stream.paths.is_none() {
			...
			// TODO:
			// We have to replace the DefaultCurrencyConversion with check at the time of handling the invoice
			// whether the amount is correct for currency case.
			if let Ok(requested_amount_msats) = invoice_request.payable_amount_msats(&DefaultCurrencyConversion) {
