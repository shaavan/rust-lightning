...
#![cfg_attr(
	not(feature = "std"),
	doc = "
    .respond_with_no_conversion(payment_paths, payment_hash, core::time::Duration::from_secs(0))?
"
)]
//! # )
...
macro_rules! invoice_builder_methods {
	...
	$self: ident, $self_type: ty, $return_type: ty, $return_value: expr, $type_param: ty $(, $self_mut: tt)?
			...
			CC::Target: $crate::offers::invoice_request::CurrencyConversion,
		{
			...
			// Compute the Offer-implied msats once (BTC or fiat), with full overflow checks.
			let offer_msats_opt: Option<u64> = match invoice_request.contents.inner.offer.amount() {
				Some(Amount::Bitcoin { amount_msats }) => {
					Some(amount_msats.checked_mul(quantity).ok_or(Bolt12SemanticError::InvalidAmount)?)
				}
				Some(Amount::Currency { iso4217_code, amount }) => {
					let unit_msats = currency_conversion
						.fiat_to_msats(iso4217_code)?
						.checked_mul(amount)
						.ok_or(Bolt12SemanticError::InvalidAmount)?;
					Some(unit_msats.checked_mul(quantity).ok_or(Bolt12SemanticError::InvalidAmount)?)
				}
				None => None,
			};

			// If the IR has an explicit amount, ensure it meets (>=) the Offer floor if present.
			if let Some(ir_msats) = invoice_request.contents.inner.amount_msats() {
				if let Some(offer_msats) = offer_msats_opt {
					if ir_msats < offer_msats {
			...
			// Otherwise, fall back to the Offer-implied amount.
			if let Some(offer_msats) = offer_msats_opt {
				return Ok(offer_msats);
			}
