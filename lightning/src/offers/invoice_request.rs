...
mod tests {
	...
	fn builds_invoice_request_with_derived_payer_signing_pubkey() {
		...
		let offer = OfferBuilder::new(recipient_pubkey(), &conversion)
			.amount_msats(1000)
			.experimental_foo(42)
			.build()
			.unwrap();
	...
	fn fails_parsing_invoice_request_with_invalid_signature() {
		...
		let mut invoice_request = OfferBuilder::new(recipient_pubkey(), &conversion)
			.amount_msats(1000)
			.build()
			.unwrap()
			.request_invoice(&expanded_key, nonce, &secp_ctx, payment_id, &conversion)
	...
	fn parses_invoice_request_with_unknown_tlv_records() {
		...
		let (mut unsigned_invoice_request, payer_keys, _) =
			OfferBuilder::new(keys.public_key(), &conversion)
				.amount_msats(1000)
				.build()
				.unwrap()
				.request_invoice(&expanded_key, nonce, &secp_ctx, payment_id, &conversion)
	...
	fn parses_invoice_request_with_experimental_tlv_records() {
		...
		let invoice_request = OfferBuilder::new(keys.public_key(), &conversion)
			.amount_msats(1000)
			.build()
			.unwrap()
			.request_invoice(&expanded_key, nonce, &secp_ctx, payment_id, &conversion)
