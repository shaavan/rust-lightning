...
use crate::offers::invoice::SigningPubkeyStrategy;
...
pub struct VerifiedInvoiceRequest<S: SigningPubkeyStrategy> {
	...
	/// The amount nature for the invoice request, which indicates how the amount should be interpreted.
	pub amount_nature: AmountNature,
...
impl<S: SigningPubkeyStrategy> VerifiedInvoiceRequest<S> {
	pub fn resolved_amount_to_use<C: CurrencyConversion>(
		&self, converter: &C,
	) -> Result<u64, Bolt12SemanticError> {
		match &self.amount_nature {
			AmountNature::BitcoinAmount(ir_only) => Ok(ir_only.invoice_request_amount),
			AmountNature::VerificationNeeded(offer_and_ir_amount) => offer_and_ir_amount.verify_amount_to_use(converter),
...
impl VerifiedInvoiceRequestEnum {
	...
	pub fn resolved_amount_to_use<C: CurrencyConversion>(&self, converter: &C) -> Result<u64, Bolt12SemanticError> {
		match self {
			VerifiedInvoiceRequestEnum::WithKeys(req) => req.resolved_amount_to_use(converter),
...
pub trait CurrencyConversion {
	fn convert_to_bitcoin(&self, iso4217_code: [u8; 3], amount: u64) -> Result<u64, Bolt12SemanticError>;
}
...
impl AmountNature {
	...
	pub(crate) fn resolve_amount_nature(
		...
		match (ir_amount, offer_amount) {
			(Some(ir_amount), Some(offer_amount)) => {
				if let Amount::Bitcoin { amount_msats } = offer_amount {
					if ir_amount < amount_msats * quantity.unwrap_or(1) { // Correction
						return Err(Bolt12SemanticError::InsufficientAmount);
					}
				}

				Ok(AmountNature::VerificationNeeded(OfferAndInvoiceRequest {
					quantity,
					offer_amount,
			...
			(Some(amount_msats), None) => {
				Ok(AmountNature::BitcoinAmount(InvoiceRequestOnly {
					invoice_request_amount: amount_msats,
				}))
			},

			(None, Some(amount)) => {
				Ok(AmountNature::DerivationNeeded(OfferOnly { quantity, offer_amount: amount }))
			},
...
/// Verifies that the request was for an offer created using the given key by checking the
	...
	>(
		...
		let amount_nature = AmountNature::resolve_amount_nature(&inner.contents)
   			.map_err(|_| ())?;
...
macro_rules! invoice_request_respond_with_derived_signing_pubkey_methods { (
	...
	pub fn respond_using_derived_keys_no_std(
		...
		let amount_msats = $self.resolved_amount_msats;
