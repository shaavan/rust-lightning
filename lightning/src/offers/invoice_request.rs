...
macro_rules! invoice_request_accessors { ($self: ident, $contents: expr) => {
	...
	/// The amount payable for the request.
	///
	/// If the invoice request explicitly sets an amount, the payable amount is
	/// exact. Otherwise, the amount is derived from [`Offer::amount`],
	/// multiplied by the requested [`quantity`], and converted to millisatoshis
	/// if the offer amount is currency-denominated.
	///
	/// Returns an error if the payable amount is missing, unsupported, or
	/// semantically invalid.
	pub fn payable_amount<CC: CurrencyConversion>(
...
impl InvoiceRequestContents {
	...
	pub(super) fn payable_amount<CC: CurrencyConversion>(
		&self, currency_conversion: &CC,
	) -> Result<MsatsRange, Bolt12SemanticError> {
		match self.inner.amount_msats() {
			Some(msats) => Ok(MsatsRange { amount_msats: msats, tolerance: 0 }),
			None => {
				let unit_msats = self
					.inner
					.offer
				...
				let amount_range = unit_msats.checked_mul(quantity)?;
...
mod tests {
	...
	fn builds_invoice_request_with_defaults() {
		...
		assert_eq!(
			invoice_request.payable_amount(&conversion).map(|amount| amount.amount_msats()),
			Ok(1000)
		);
	...
	fn builds_invoice_request_without_amount() {
		...
		assert_eq!(invoice_request.amount_msats(), None);
		assert_eq!(
			invoice_request.payable_amount(&unsupported_conversion),
		...
		assert_eq!(invoice_request.amount_msats(), None);
		assert_eq!(
			invoice_request.payable_amount(&conversion),
