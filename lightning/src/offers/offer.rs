...
/// Time units for recurrence periods.
...
pub struct RecurrenceData {
	...
	invoice_request_start: Option<u32>,

	/// The next expected recurrence period index.
	///
	/// Instead of storing the last seen `recurrence_counter`, we store the
	/// *next expected index*. This simplifies validation:
	/// `incoming_counter == next_payable_index`
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(super) struct RecurrenceFields {
...
impl TryFrom<FullOfferTlvStream> for OfferContents {
	...
	fn try_from(tlv_stream: FullOfferTlvStream) -> Result<Self, Self::Error> {
		...
		/// During deserialising we have to take a different set of assumption than we took during serialising.
		/// 
		/// During serialising, our node handles the serialising, so it can 
