...
impl OfferContents {
	...
	pub(super) fn check_amount_msats_for_quantity<CC: CurrencyConversion>(
		...
		let expected_amount_msats = self
			...
			.map(|unit_msats| unit_msats.checked_mul(quantity).map(|range| range.amount_msats()))
			.transpose()?;
		...
		let total_amount_msats = match (requested_amount_msats, expected_amount_msats) {
			...
			// Enforce that the requested amount satisfies the locally resolved
			// minimum. Tolerance only applies when comparing independently
			// resolved conversion snapshots.
