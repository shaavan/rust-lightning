...
impl Recurrence {
	...
	fn period_start_from_offset(&self, basetime: u64, period_offset: u64) -> u64 {
		match self.time_unit {
			TimeUnit::Seconds => self.seconds_period_start(basetime, period_offset),
			TimeUnit::Days => self.days_period_start(basetime, period_offset),
			TimeUnit::Months => self.months_period_start(basetime, period_offset),
		}
	}

	fn seconds_period_start(&self, basetime: u64, period_offset: u64) -> u64 {
		let seconds_to_add = u64::from(self.period)
	...
	fn days_period_start(&self, basetime: u64, period_offset: u64) -> u64 {
		let (base_day, seconds_within_day) = Self::split_timestamp(basetime);
		let days_to_add = u64::from(self.period)
			.checked_mul(period_offset)
	...
	fn months_period_start(&self, basetime: u64, period_offset: u64) -> u64 {
		let (base_day, seconds_within_day) = Self::split_timestamp(basetime);
		let (year, month, day) = Self::civil_from_days(base_day);
		let months_to_add = u64::from(self.period)
			.checked_mul(period_offset)
	...
	pub(super) fn as_tlv_stream(&self) -> FullOfferTlvStreamRef<'_> {
		...
		let (
			...
			match &r.recurrence_type {
				RecurrenceType::Optional => (None, Some(&r.recurrence), None, paywindow, limit),
				RecurrenceType::Compulsory(base) => {
					(Some(&r.recurrence), None, base.as_ref(), paywindow, limit)
				},
			}
		});
...
impl TryFrom<FullOfferTlvStream> for OfferContents {
	...
	fn try_from(tlv_stream: FullOfferTlvStream) -> Result<Self, Self::Error> {
		...
		let recurrence_fields = match (recurrence_compulsory, recurrence_optional, recurrence_base)
		{
			...
			(Some(period), None, base) => Some(RecurrenceFields {
				recurrence: period,
...
mod tests {
	...
	use super::{
		...
		RecurrenceBase, RecurrenceFields, RecurrenceLimit, RecurrencePaywindow, RecurrenceType,
		TimeUnit, EXPERIMENTAL_OFFER_TYPES, OFFER_TYPES,
	};
	...
	fn builds_offer_with_recurrence() {
		...
		let mut builder = OfferBuilder::new(pubkey(42));
		builder.offer.recurrence_fields = Some(RecurrenceFields {
			recurrence,
			recurrence_type: RecurrenceType::Optional,
		...
		assert_eq!(
			offer.recurrence_fields(),
			Some(RecurrenceFields {
				recurrence,
				recurrence_type: RecurrenceType::Optional,
				recurrence_paywindow: Some(paywindow),
				recurrence_limit: Some(limit),
	...
	fn computes_recurrence_period_times() {
		let seconds_recurrence = Recurrence { time_unit: TimeUnit::Seconds, period: 3_600 };
		...
		let days_recurrence = Recurrence { time_unit: TimeUnit::Days, period: 7 };
		...
		let months_recurrence = Recurrence { time_unit: TimeUnit::Months, period: 1 };
