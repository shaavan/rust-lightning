	...
	MR::Target: MessageRouter,
{
		...
		R::Target: Router,
	{
		...
		let response = if invoice_request.keys.is_some() {
			let builder = invoice_request.respond_using_derived_keys(payment_paths, payment_hash, created_at, recurrence_basetime);
			builder
				.map(InvoiceBuilder::<DerivedSigningPubkey>::from)
				.and_then(|builder| builder.allow_mpp().build_and_sign(secp_ctx))
				.map_err(InvoiceError::from)
		} else {
			let builder = invoice_request.respond_with(payment_paths, payment_hash, created_at, recurrence_basetime);
			builder
