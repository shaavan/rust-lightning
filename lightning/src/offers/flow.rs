	...
	L::Target: Logger,
{
	...
	pub fn create_inbound_payment(&self, min_value_msat: Option<u64>, invoice_expiry_delta_secs: u32,
		...
		inbound_payment::create(&self.inbound_payment_key, min_value_msat, invoice_expiry_delta_secs,
			&self.entropy_source, self.commons.get_highest_seen_timestamp(),
	...
	pub(crate) fn duration_since_epoch(&self) -> Duration {
		#[cfg(not(feature = "std"))]
		let now = Duration::from_secs(self.commons.get_highest_seen_timestamp());
		#[cfg(feature = "std")]
	...
	fn handle_message(
		...
		match message {
			OffersMessage::InvoiceRequest(invoice_request) => {
				...
				#[cfg(not(feature = "std"))]
				let created_at = Duration::from_secs(self.commons.get_highest_seen_timestamp());
