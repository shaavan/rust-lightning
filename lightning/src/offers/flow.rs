...
pub struct OffersMessageFlow<'a, ES: Deref, MR: Deref, R: Deref>
where
...
impl<'a, ES: Deref, MR: Deref, R: Deref> OffersMessageFlow<'a, ES, MR, R>
where
	ES::Target: EntropySource,
	MR::Target: MessageRouter,
	R::Target: Router,
{
	...
	pub fn new(
		chain_hash: ChainHash, best_block: BestBlock, our_network_pubkey: PublicKey, current_timestamp: u32,
		inbound_payment_key: inbound_payment::ExpandedKey, entropy_source: ES, message_router: &'a MR,
		router: &'a R,
	) -> Self {
...
impl<'a, ES: Deref, MR: Deref, R: Deref> chain::Listen for OffersMessageFlow<'a, ES, MR, R>
where
	ES::Target: EntropySource,
	MR::Target: MessageRouter,
	R::Target: Router,
	...
	R::Target: Router,
{
	...
	#[cfg(async_payments)]
	pub fn verify_async_context(&self, context: AsyncPaymentsContext) -> Result<Option<PaymentId>, ()> {
		match context {
	...
	pub fn create_invoice_request_builder<'b>(
		&'b self, offer: &'b Offer, nonce: Nonce, quantity: Option<u64>, amount_msats: Option<u64>,
		payer_note: Option<String>, human_readable_name: Option<HumanReadableName>,
		payment_id: PaymentId,
	) -> Result<InvoiceRequestBuilder<'b, 'b, secp256k1::All>, Bolt12SemanticError> {
		let expanded_key = &self.inbound_payment_key;
		let secp_ctx = &self.secp_ctx;
	...
	#[cfg(async_payments)]
	pub fn create_static_invoice_builder<'b>(
		&'b self, offer: &'b Offer, offer_nonce: Nonce, relative_expiry: Option<Duration>,
		usable_channels: Vec<ChannelDetails>, peers: Vec<MessageForwardNode>,
	) -> Result<StaticInvoiceBuilder<'b>, Bolt12SemanticError> {
		let expanded_key = &self.inbound_payment_key;
		let entropy = &*self.entropy_source;
		let secp_ctx = &self.secp_ctx;
	...
	pub fn create_invoice_builder_from_refund<'b>(
		&'b self, refund: &'b Refund, payment_hash: PaymentHash, payment_secret: PaymentSecret,
		usable_channels: Vec<ChannelDetails>,
	) -> Result<InvoiceBuilder<'b, DerivedSigningPubkey>, Bolt12SemanticError> {
		let expanded_key = &self.inbound_payment_key;
		let entropy = &*self.entropy_source;
