...
use crate::offers::invoice::{
	self, Bolt12Invoice, DerivedSigningPubkey, ExplicitSigningPubkey, InvoiceBuilder, UnsignedBolt12Invoice, DEFAULT_RELATIVE_EXPIRY
};
	...
	MR::Target: MessageRouter,
{
	...
	pub fn synchornously_handle_invoice(&self, invoice: Bolt12Invoice, payment_id: PaymentId) -> Result<Option<Bolt12Invoice>, ()> {
		if self.should_trigger_invoice_event(&invoice) {
			let amount_source = match invoice.get_offer_amount() {
				Ok(Some(offer_amount)) => Bolt12InvoiceAmountSource::ForOffer { amount: IREventAmount::OfferOnly { offer_amount } },
				Ok(None) => Bolt12InvoiceAmountSource::ForRefund { refund_amount: invoice.get_precursor_amount().unwrap_or(0) },
				Err(_) => return Err(()), // Invalid offer amount
			};
