...
fn do_test_simple_monitor_temporary_update_fail(disconnect: bool) {
	...
	match events_3[0] {
		Event::PaymentClaimable { ref payment_hash, ref purpose, amount_msat, receiver_node_id, ref via_channel_id_pairs, .. } => {
			assert_eq!(payment_hash_1, *payment_hash);
			assert_eq!(amount_msat, 1_000_000);
			assert_eq!(receiver_node_id.unwrap(), nodes[1].node.get_our_node_id());
			assert_eq!(via_channel_id_pairs.len(), 1);
			assert_eq!(via_channel_id_pairs.last().unwrap().0, channel_id);
			match &purpose {
...
fn test_monitor_update_fail_cs() {
	...
	match events[0] {
		Event::PaymentClaimable { payment_hash, ref purpose, amount_msat, receiver_node_id, ref via_channel_id_pairs, .. } => {
...
fn test_monitor_update_fail_claim() {
	...
	match events[0] {
		Event::PaymentClaimable { ref payment_hash, ref purpose, amount_msat, receiver_node_id, ref via_channel_id_pairs, .. } => {
			...
			assert_eq!(via_channel_id_pairs.len(), 1);
			assert_eq!(*via_channel_id_pairs.last().unwrap(), (channel_id, Some(42)));
			match &purpose {
				PaymentPurpose::Bolt11InvoicePayment { payment_preimage, payment_secret, .. } => {
					assert!(payment_preimage.is_none());
					assert_eq!(payment_secret_2, *payment_secret);
