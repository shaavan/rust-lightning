...
pub fn create_node_cfgs_with_persisters<'a>(node_count: usize, chanmon_cfgs: &'a Vec<TestChanMonCfg>, persisters: Vec<&'a impl Persist<TestChannelSigner>>) -> Vec<NodeCfg<'a>> {
	...
	for i in 0..node_count {
		...
		// let flow = TestOffersMessageFlow::new(network, &message_router, chanmon_cfgs[i].keys_manager.get_node_id(Recipient::Node).unwrap(), genesis_block.header.time, chanmon_cfgs[i].keys_manager.get_inbound_payment_key(), &chanmon_cfgs[i].keys_manager);
		let seed = [i as u8; 32];
		nodes.push(NodeCfg {
