...
fn ignore_duplicate_invoice() {
	...
	let res = claim_payment_along_route(ClaimAlongRouteArgs::new(
		sender,
		&[route[0]],
		payment_preimage
	).with_dummy_tlvs(&dummy_tlvs)).0;
...
fn async_receive_flow_success() {
	...
	let keysend_preimage = extract_payment_preimage(&claimable_ev);
	let (res, _) =
		claim_payment_along_route(ClaimAlongRouteArgs::new(&nodes[0], route, keysend_preimage).with_dummy_tlvs(&dummy_tlvs));
...
fn invoice_server_is_not_channel_peer() {
	...
	let claimable_ev = do_pass_along_path(args).unwrap();
	let keysend_preimage = extract_payment_preimage(&claimable_ev);
	let res = claim_payment_along_route(ClaimAlongRouteArgs::new(sender, route, keysend_preimage).with_dummy_tlvs(&dummy_tlvs));
...
fn async_payment_mpp() {
	...
	let expected_route: &[&[&Node]] = &[&[&nodes[1], &nodes[3]], &[&nodes[2], &nodes[3]]];
	claim_payment_along_route(ClaimAlongRouteArgs::new(sender, expected_route, keysend_preimage).with_dummy_tlvs(&dummy_tlvs));
}
