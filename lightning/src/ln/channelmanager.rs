	...
	fn initiate_async_payment(
		...
		PersistenceNotifierGuard::optionally_notify(self, || {
...

			{
				...
				if self.flow.enqueue_held_htlc_available(entropy, invoice, payment_id, Self::get_peers_for_blinded_path(peer_state_iter)).is_err() {
...
macro_rules! create_offer_builder { ($self: ident, $builder: ty) => {
	...
	pub fn create_offer_builder(
		...
		let builder = {
			...
			$self.flow.create_offer_builder(
				entropy, absolute_expiry, Self::get_peers_for_blinded_path(peer_state_iter)
			)?
		};
...
macro_rules! create_refund_builder { ($self: ident, $builder: ty) => {
	...
	pub fn create_refund_builder(
		...
		let builder = {
			...
			$self.flow.create_refund_builder(
				entropy, amount_msats, absolute_expiry,
				payment_id, Self::get_peers_for_blinded_path(peer_state_iter)
			)?
		};
	...
	pub fn create_static_invoice_builder<'a>(
		...
		let peers = {
			...
			Self::get_peers_for_blinded_path(
				per_peer_state.iter().map(|(node_id, peer_state)| {
					(node_id, peer_state.lock().unwrap())
				})
			).collect::<Vec<_>>()
		};
	...
	fn pay_for_offer_intern<CPP: FnOnce(&InvoiceRequest, Nonce) -> Result<(), Bolt12SemanticError>>(
...

		{
			...
			self.flow.enqueue_invoice_request(
				...
				Self::get_peers_for_blinded_path(peer_state_iter)
			)?;
		}
		...
		match self.create_inbound_payment(Some(amount_msats), relative_expiry, None) {
			Ok((payment_hash, payment_secret)) => {
...

				{
					...
					self.flow.enqueue_invoice(entropy, invoice.clone(), refund, Self::get_peers_for_blinded_path(peer_state_iter))?;
				}
			...
			.release_invoice_requests_awaiting_invoice()
		{
...

			{
				...
				if self.flow.enqueue_invoice_request(
					invoice_request, payment_id, nonce, Self::get_peers_for_blinded_path(peer_state_iter)
				).is_err() {
