...
pub enum PendingHTLCRouting {
	...
	Dummy {
		/// The onion which should be included in the next created HTLC that's sent back to us,
		/// telling the our hop what to do with the HTLC.
		onion_packet: msgs::OnionPacket,

		// - short channel id not needed, since htlc is to us. Edit. However SCID can also be alias,
		// so why not use it to our advantage, where reading SCID, we can understand that it's our ID.
		// - blinded not needed since dummy hops only makes sense when used with blinded path.
		// - We keep the incoming_cltv_expiry, for this data is also relevant for the real final
		// receive tlvs too.
		/// The short channel ID of the channel which we were instructed to forward this HTLC to.
		///
		/// This could be a real on-chain SCID, an SCID alias, or some other SCID which has meaning
		...
		// Compare to Forward, or Trampoline Forward, this is the only information we require.
		// Comparing to peel_onion_message, blinding_override will always be none.
		// And dummy hops are only relevant to blinded case, so `msg.blinded_point`
		// will always be Some
		// Edit. Maybe true, but for being conservative sake. We go with BlindedForward
		/// Set if this HTLC is being forwarded within a blinded path.
		blinded: BlindedForward,

		/// The absolute CLTV of the inbound HTLC
		incoming_cltv_expiry: Option<u32>,
		// - hold htlc, doesn't make sense too, for why and for what are we holding. After all,
		// we are the 'real' reciever.
	},

	// We don't introduce TrampolineDummy for that doesn't makes sense too, because there's no TrampolineReceive either.
	// Ones the HTLC has reached 'us', the real receiver, what difference does it make if it came from Trampoline path,
	// or normal path.
	// Edit:
	// The above is wrong. We do need to have a Trampoline Dummy. How else are we gonna converse the information, when creating
	// the final receive hop whether it was a BlindedReceive, or TrampolineBlindedReceive?
	TrampolineDummy {
		/// The onion shared secret we build with the sender (or the preceding Trampoline node) used
		/// to decrypt the onion.
		///
