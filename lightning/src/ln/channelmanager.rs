...
use crate::offers::invoice_request::{CurrencyConversion, InvoiceRequest, VerifiedInvoiceRequestEnum};
...
/// This is not exported to bindings users as type aliases aren't supported in most languages.
#[cfg(not(c_bindings))]
pub type SimpleRefChannelManager<'a, 'b, 'c, 'd, 'e, 'f, 'g, 'h, 'i, 'j, M, T, F, L> = ChannelManager<
	&'a M,
	&'b T,
	&'c KeysManager,
	&'c KeysManager,
	...
	L::Target: Logger,
{
	...
	type L = L;
	#[rustfmt::skip]
	fn get_cm(&self) -> &ChannelManager<M, T, ES, NS, SP, F, R, MR, CC, L> { self }
}
	...
	L::Target: Logger,
{
	...
	pub fn new(
		...
		let flow = OffersMessageFlow::new(
			...
			secp_ctx.clone(), message_router, currency_conversion,
		);
	...
	L::Target: Logger,
{
	...
	pub fn new(
		...
		chain_monitor: M, tx_broadcaster: T, router: R, message_router: MR, currency_conversion: CC,
		logger: L, default_config: UserConfig,
		...
		Self {
			...
			tx_broadcaster, router, message_router, currency_conversion, logger, default_config,
...
{
	#[rustfmt::skip]
	fn read<Reader: io::Read>(reader: &mut Reader, args: ChannelManagerReadArgs<'a, M, T, ES, NS, SP, F, R, MR, CC, L>) -> Result<Self, DecodeError> {
		let (blockhash, chan_manager) = <(BlockHash, ChannelManager<M, T, ES, NS, SP, F, R, MR, CC, L>)>::read(reader, args)?;
...
{
	#[rustfmt::skip]
	fn read<Reader: io::Read>(reader: &mut Reader, mut args: ChannelManagerReadArgs<'a, M, T, ES, NS, SP, F, R, MR, CC, L>) -> Result<Self, DecodeError> {
		...
		let flow = OffersMessageFlow::new(
			...
			secp_ctx.clone(), args.message_router, args.currency_conversion
		);
