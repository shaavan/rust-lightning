	...
	L::Target: Logger,
{
	...
	active_outbound_recurrence_sessions: Mutex<HashMap<PublicKey, OutboundRecurrenceSessionData>>,
			...
			// -----------------------------------------------------------------------------
			{
				...
				sessions.retain(|_, data| {
					match (data.invoice_recurrence_basetime, data.next_trigger_time) {
						...
						// -----------------------------------------------------------------
						(Some(basetime), Some(trigger_time)) => {
							...
							let fields = match data.offer.recurrence_fields() {
								...
								None => {
									debug_assert!(
										false,
										"Active recurrence session without recurrence fields"
									);
									return false;
								}
							};
							...
							let paywindow_secs = fields
								...
								.unwrap_or(data.recurrence.period_length_secs());

							let window_end = data
								.recurrence
								.start_time(basetime, data.next_recurrence_counter)
								+ paywindow_secs;
	...
	L::Target: Logger,
{
	...
	/// Similar to [`pay_for_offer`] but used for sending primary invoice request
	/// for recurrence offer, setting up active outbound recurrence session.
		...
		let recurrence_data = if let Some(fields) = offer.recurrence_fields() {
			...
			let (invoice_recurrence_basetime, next_trigger_time) =
				if let Some(base) = recurrence_basetime {
					...
					let period_number = recurrence_start.unwrap_or(0);
					let next_trigger = fields.recurrence.start_time(basetime, period_number);
	...
	fn handle_message(
		...
		match message {
			...
			OffersMessage::Invoice(invoice) => {
				...
				let existing_session = sessions.get_mut(&invoice.payer_signing_pubkey());
					...
					(Some(fields), Some(counter), Some(invoice_basetime), Some(data)) if counter == 0 => {
						match data.invoice_recurrence_basetime {
							...
							None => {
								...
								// Compute the next expected trigger time for the recurrence schedule.
								let trigger_time =
									fields.recurrence.start_time(invoice_basetime, period_number);
