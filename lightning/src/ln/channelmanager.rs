	...
	L::Target: Logger,
{
	...
	/// Mutex-protected counter tracking the number of times `timer_tick_occurred()` is called.
	/// It controls the execution of specific code blocks after a certain number of calls.
	timer_tick_call_counter: Mutex<usize>,
	...
	L::Target: Logger,
{
	...
	pub fn new(
		...
		ChannelManager {
			...
			timer_tick_call_counter: Mutex::new(0),
	...
	pub fn timer_tick_occurred(&self) {
		let mut call_count = self.timer_tick_call_counter.lock().unwrap();
		...
		*call_count += 1;
		...
		let _ = self.retry_invoice_request_messages();

		// Execute the internal code only if the call count is a multiple of factor
		if *call_count % factor == 0 {
			PersistenceNotifierGuard::optionally_notify(self, || {
				let mut should_persist = NotifyOption::SkipPersistNoEvents;
