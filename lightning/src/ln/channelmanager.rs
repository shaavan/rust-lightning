...
macro_rules! create_refund_builder { ($self: ident, $builder: ty) => {
	...
	pub fn create_refund_builder(
		...
		let recipient_data = RecipientData::OffersContext(
			OffersData::WithPaymentId(payment_id)
		);
		let path = $self.create_blinded_path_using_absolute_expiry(Some(absolute_expiry), Some(recipient_data))
			.map_err(|_| Bolt12SemanticError::MissingPaths)?;
	...
	L::Target: Logger,
{
	...
	fn create_blinded_path_using_absolute_expiry(
		&self, absolute_expiry: Option<Duration>, recipient_data: Option<RecipientData>
	) -> Result<BlindedPath, ()> {
		let now = self.duration_since_epoch();
		let max_short_lived_absolute_expiry = now.saturating_add(MAX_SHORT_LIVED_RELATIVE_EXPIRY);

		if absolute_expiry.unwrap_or(Duration::MAX) <= max_short_lived_absolute_expiry {
			self.create_compact_blinded_path(recipient_data)
		} else {
			self.create_blinded_path(recipient_data)
		}
	}
	...
	///
	/// Errors if the `MessageRouter` errors or returns an empty `Vec`.
	fn create_compact_blinded_path(&self, recipient_data: Option<RecipientData>) -> Result<BlindedPath, ()> {
		let recipient = self.get_our_node_id();
		let secp_ctx = &self.secp_ctx;
		...
		self.router
			.create_compact_blinded_paths(recipient, recipient_data, peers, secp_ctx)
			.and_then(|paths| paths.into_iter().next().ok_or(()))
	}
