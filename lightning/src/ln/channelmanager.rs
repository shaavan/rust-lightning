...
use crate::routing::router::{BlindedTail, InFlightHtlcs, Path, Payee, PaymentParameters, Route, RouteParameters, UserParameters, Router};
...
/// # use lightning::routing::router::UserParameters;
/// #
...
/// #     payer_note: Option<String>, retry: Retry, user_params: Option<UserParameters>
/// # ) {
/// # let channel_manager = channel_manager.get_cm();
/// let payment_id = PaymentId([42; 32]);
/// match channel_manager.pay_for_offer(
///     offer, quantity, amount_msats, payer_note, payment_id, retry, user_params
/// ) {
...
macro_rules! create_refund_builder { ($self: ident, $builder: ty) => {
	...
	pub fn create_refund_builder(
		...
		let user_params = max_total_routing_fee_msat.map(
			|fee_msat| UserParameters::new()
		...
		$self.pending_outbound_payments
			.add_new_awaiting_invoice(
				payment_id, expiration, retry_strategy, user_params, None,
			)
	...
	L::Target: Logger,
{
	...
	pub fn pay_for_offer(
		...
		user_params: Option<UserParameters>
	) -> Result<(), Bolt12SemanticError> {
		let expanded_key = &self.inbound_payment_key;
		let entropy = &*self.entropy_source;
		let secp_ctx = &self.secp_ctx;
		...
		self.pending_outbound_payments
			.add_new_awaiting_invoice(
				payment_id, expiration, retry_strategy, user_params,
