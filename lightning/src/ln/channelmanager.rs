	...
	L::Target: Logger,
{
	...
	pub fn get_response_for_invoice_request(&self, invoice_request: InvoiceRequest, context: Option<&MessageContext>, custom_amount_msats: Option<u64>) -> Result<Bolt12Invoice, InvoiceError> {
		let secp_ctx = &self.secp_ctx;
		let expanded_key = &self.inbound_payment_key;
	...
	fn get_response_for_verified_invoice_request(&self, invoice_request: &VerifiedInvoiceRequest, custom_amount_msats: Option<u64>) -> Result<Bolt12Invoice, InvoiceError> {
		let amount_msats = match InvoiceBuilder::<DerivedSigningPubkey>::amount_msats(
			&invoice_request.inner, custom_amount_msats
		) {
			Ok(amount_msats) => amount_msats,
			Err(error) => return Err(InvoiceError::from(error)),
		};
