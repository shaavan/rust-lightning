...
use crate::offers::parse::{Bolt12Error, Bolt12SemanticError, Bolt12ValidationError};
	...
	L::Target: Logger,
{
	...
	fn get_response_for_invoice_request(&self, invoice_request: InvoiceRequest, context: Option<&MessageContext>, custom_amount_msats: Option<u64>) -> Result<OffersMessage, Bolt12Error> {
		let secp_ctx = &self.secp_ctx;
		let expanded_key = &self.inbound_payment_key;
		...
		let nonce = match context {
			...
			_ => return Err(Bolt12Error::Validation(Bolt12ValidationError::InvalidMetadata)),
		};
		...
		let invoice_request = match nonce {
			Some(nonce) => match invoice_request.verify_using_recipient_data(
				*nonce, expanded_key, secp_ctx,
				...
				Err(()) => return Err(Bolt12Error::Validation(Bolt12ValidationError::VerificationFailed)),
			},
	...
	fn get_response_for_verified_invoice_request(&self, invoice_request: &VerifiedInvoiceRequest, custom_amount_msats: Option<u64>) -> Result<OffersMessage, Bolt12Error> {
		let amount_msats = match InvoiceBuilder::<DerivedSigningPubkey>::amount_msats(
			&invoice_request.inner, custom_amount_msats
		) {
			Ok(amount_msats) => amount_msats,
			Err(error) => return Err(Bolt12Error::Semantic(error)),
		};
		...
		let (payment_hash, payment_secret) = match self.create_inbound_payment(
			...
			Err(()) => return Err(Bolt12Error::Semantic(Bolt12SemanticError::InvalidAmount)),
		};
		...
		let payment_paths = match self.create_blinded_payment_paths(
			...
			Err(()) => return Err(Bolt12Error::Semantic(Bolt12SemanticError::MissingPaths)),
		};
		...
		match response {
			...
			Err(error) => {
				match error {
					...
					Bolt12Error::Validation(_) => {},
					Bolt12Error::Semantic(error) => {
		...
		match message {
			OffersMessage::InvoiceRequest(invoice_request) => {
				...
				match response {
					...
					Err(Bolt12Error::Semantic(error)) => Some((OffersMessage::InvoiceError(error.into()), responder.respond())),
					Err(_) => None,
				}
			},
			OffersMessage::Invoice(invoice) => {
