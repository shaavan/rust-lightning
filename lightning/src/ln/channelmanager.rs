...
#[cfg(feature = "dnssec")]
use {
	crate::onion_message::dns_resolution::{DNSResolverMessage, DNSResolverMessageHandler, DNSSECQuery, DNSSECProof, OMNameResolver},
	...
	L::Target: Logger,
{
	...
	fn initiate_async_payment(
		...
		PersistenceNotifierGuard::optionally_notify(self, || {
			...
			if self.flow.enqueue_async_payment_messages(entropy, invoice, payment_id, self.get_peers_for_blinded_path()).is_err() {
	...
	L::Target: Logger,
{
	...
	pub fn create_static_invoice_builder<'a>(
		...
		let entropy = &*self.entropy_source;
		self.flow.create_static_invoice_builder(&self.router, entropy, offer, offer_nonce, relative_expiry, self.list_usable_channels(), self.get_peers_for_blinded_path())
	}
		...
		match message {
			OffersMessage::InvoiceRequest(invoice_request) => {
				...
				let (response, context) = self.flow.create_response_for_invoice_request(
					&self.node_signer, &self.router, entropy, invoice_request, amount_msats, payment_hash, payment_secret,
