	...
	fn write<W: Writer>(&self, w: &mut W) -> Result<(), io::Error> {
		match self {
			...
			Self::Receive {
				...
				let user_custom_tlv = if !user_custom_tlvs.is_empty() {
					Some((65539, user_custom_tlvs.to_vec()))
				} else { None };
			...
			Self::BlindedReceive {
				...
				intro_node_blinding_point, keysend_preimage, ref sender_custom_tlvs, user_custom_tlvs
			} => {
				// We need to update [`ln::outbound_payment::RecipientOnionFields::with_custom_tlvs`]
				// to reject any reserved types in the experimental range if new ones are ever
				// standardized.
				...
				let mut sender_custom_tlvs: Vec<&(u64, Vec<u8>)> = sender_custom_tlvs
					.iter()
					.chain(keysend_tlv.iter())
					.chain(user_custom_tlv.iter())
					.collect();
...
impl<NS: Deref> ReadableArgs<(Option<PublicKey>, NS)> for InboundOnionPayload where NS::Target: NodeSigner {
	fn read<R: Read>(r: &mut R, args: (Option<PublicKey>, NS)) -> Result<Self, DecodeError> {
		...
		// `user_custom_tlvs` now contains any entries with TLV type `65539u64`. Extract the data or use an empty Vec.
