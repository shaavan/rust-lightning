...
#[test]
fn no_double_pay_with_stale_channelmanager_() {
	// This tests the following bug:
	// - An outbound payment is AwaitingInvoice
	// - We receive an invoice and lock the HTLCs into the relevant ChannelMonitors
	// - The monitors are successfully persisted, but the ChannelManager fails to persist, so the
	...
	let amt_msat = nodes[0].node.list_usable_channels()[0].next_outbound_htlc_limit_msat - 1; // Not forcing MPP

	// Create offer with no blinded path
	let offer = nodes[1].node
		.create_offer_builder(None).unwrap()
		.clear_paths()
		.amount_msats(amt_msat)
	...
	// node 0 pay_for_offer creating IR
	nodes[0].node.pay_for_offer(&offer, None, None, None, payment_id, Retry::Attempts(1), RouteParametersConfig::default()).unwrap();
	expect_recent_payment!(nodes[0], RecentPaymentDetails::AwaitingInvoice, payment_id);

	// Get the IR, and handle it from node 1 end
	let invreq_om = nodes[0].onion_messenger.next_onion_message_for_peer(bob_id).unwrap();
	nodes[1].onion_messenger.handle_onion_message(alice_id, &invreq_om);
	...
	// node 1 handle the onion message, creating invoice, we handle invoice on node 0 end.
	let invoice_om = nodes[1].onion_messenger.next_onion_message_for_peer(alice_id).unwrap();
	nodes[0].onion_messenger.handle_onion_message(bob_id, &invoice_om);

	let payment_hash = extract_invoice(&nodes[0], &invoice_om).0.payment_hash();

	// We get the msg sent event generated by node[0]
	let mut events = nodes[0].node.get_and_clear_pending_msg_events();
	assert_eq!(events.len(), 1);
	check_added_monitors!(nodes[0], 1);

	// We lock the HTLCs to be sent along the path.
	let path = &[&nodes[1]];
	let ev = remove_first_msg_event_to_node(&bob_id, &mut events);
	...
	// Now node 0 payment is in the Pending state, accroding to it, and payment is claimable for node 1
	expect_recent_payment!(nodes[0], RecentPaymentDetails::Pending, payment_id);
	match get_event!(nodes[1], Event::PaymentClaimable) {
		Event::PaymentClaimable { .. } => {},
		_ => panic!("No Event::PaymentClaimable"),
	...
	// Now we reload with an outdated ChannelManager, simulating that the latest state of ChannelManager
	// (aka payment deatils in `Pending`) state was not persisted.
	let monitor_0 = get_monitor!(nodes[0], chan_id_0).encode();
	reload_node!(nodes[0], &alice_chan_manager_serialized, &[&monitor_0], persister, chain_monitor, alice_deserialized);
	// The stale manager results in closing the channels.
	check_closed_event!(nodes[0], 1, ClosureReason::OutdatedChannelManager, [bob_id], 10_000_000);
	check_added_monitors!(nodes[0], 1);

	nodes[1].node.force_close_all_channels_without_broadcasting_txn("Channel force-closed".to_string());
	check_closed_event!(nodes[1], 1, ClosureReason::HolderForceClosed { broadcasted_latest_txn: Some(false) }, [alice_id], 10_000_000);
	check_added_monitors!(nodes[1], 1);
	let _msg_events = nodes[1].node.get_and_clear_pending_msg_events();
	
	let new_chan_0 = create_announced_chan_between_nodes_with_value(&nodes, 0, 1, 10_000_000, 1_000_000_000);
}
