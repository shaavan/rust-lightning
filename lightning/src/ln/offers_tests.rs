...
use crate::onion_message::messenger::{BlindedPathType, Destination, PeeledOnion};
...
fn prefers_non_tor_nodes_in_blinded_paths() {
	...
	*node_cfgs[1].override_init_features.borrow_mut() = Some(features);
	...
	let offer = bob.node
		.create_offer_builder(None, Some(BlindedPathType::Full)).unwrap()
...
fn creates_short_lived_offer() {
	...
	let offer = alice.node
		.create_offer_builder(Some(absolute_expiry), Some(BlindedPathType::Compact)).unwrap()
		.build().unwrap();
...
fn creates_long_lived_offer() {
	...
	let offer = alice.node
		.create_offer_builder(Some(absolute_expiry), Some(BlindedPathType::Full))
		.unwrap()
		.build().unwrap();
	assert_eq!(offer.absolute_expiry(), Some(absolute_expiry));
	assert!(!offer.paths().is_empty());
...
fn creates_short_lived_refund() {
	...
	let refund = bob.node
		.create_refund_builder(10_000_000, absolute_expiry, payment_id, Some(BlindedPathType::Compact), Retry::Attempts(0), None)
		.unwrap()
		.build().unwrap();
...
fn creates_long_lived_refund() {
	...
	let payment_id = PaymentId([1; 32]);
	let refund = bob.node
		.create_refund_builder(10_000_000, absolute_expiry, payment_id, Some(BlindedPathType::Full), Retry::Attempts(0), None)
		.unwrap()
		.build().unwrap();
	assert_eq!(refund.absolute_expiry(), Some(absolute_expiry));
	assert!(!refund.paths().is_empty());
...
fn creates_and_pays_for_offer_using_two_hop_blinded_path() {
	...
	*node_cfgs[1].override_init_features.borrow_mut() = Some(features);
	...
	let offer = alice.node
		.create_offer_builder(None, Some(BlindedPathType::Full))
		.unwrap()
		.amount_msats(10_000_000)
		.build().unwrap();
...
fn fails_creating_or_paying_for_offer_without_connected_peers() {
	...
	let absolute_expiry = alice.node.duration_since_epoch() + MAX_SHORT_LIVED_RELATIVE_EXPIRY;
	match alice.node.create_offer_builder(Some(absolute_expiry), Some(BlindedPathType::Full)) {
	...
	let offer = alice.node
		.create_offer_builder(Some(absolute_expiry), Some(BlindedPathType::Full)).unwrap()
		.amount_msats(10_000_000)
		.build().unwrap();
...
fn fails_creating_refund_or_sending_invoice_without_connected_peers() {
	...
	*node_cfgs[1].override_init_features.borrow_mut() = Some(features);
	...
	match david.node.create_refund_builder(
		10_000_000, absolute_expiry, payment_id, Some(BlindedPathType::Full), Retry::Attempts(0), None
	) {
		Ok(_) => panic!("Expected error"),
		Err(e) => assert_eq!(e, Bolt12SemanticError::MissingPaths),
	}
...
fn fails_creating_refund_with_duplicate_payment_id() {
	...
	assert!(
		nodes[0].node.create_refund_builder(
			10_000, absolute_expiry, payment_id, Some(BlindedPathType::Full), Retry::Attempts(0), None
		).is_ok()
	);
