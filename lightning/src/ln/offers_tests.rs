...
fn create_offer_with_no_blinded_path() {
	...
	let offer = alice.node
		.create_offer_builder_using_router(&router, None).unwrap()
...
fn creates_long_lived_offer() {
	...
	let offer = alice.node
		.create_offer_builder_using_router(&router, None)
		.unwrap()
		.build().unwrap();
	assert!(!offer.paths().is_empty());
	for path in offer.paths() {
...
fn fails_creating_or_paying_for_offer_without_connected_peers() {
	...
	match alice.node.create_offer_builder(None) {
...
fn fails_paying_invoice_with_unknown_required_features() {
	...
	*node_cfgs[1].override_init_features.borrow_mut() = Some(features);
	...
	let invoice = invoice_request
		...
		.respond_using_derived_keys(payment_paths, payment_hash, created_at, None).unwrap()
