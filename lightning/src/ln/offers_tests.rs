...
use crate::onion_message::messenger::{BlindedPathType, Destination, PeeledOnion, PATHS_PLACEHOLDER};
...
fn prefers_non_tor_nodes_in_blinded_paths() {
	...
	*node_cfgs[1].override_init_features.borrow_mut() = Some(features);
	...
	let offer = bob.node
		.create_offer_builder(Some(BlindedPathType::Full { paths: PATHS_PLACEHOLDER })).unwrap()
...
fn creates_short_lived_offer() {
	...
	let offer = alice.node
		.create_offer_builder(Some(BlindedPathType::Compact { paths: PATHS_PLACEHOLDER })).unwrap()
...
fn creates_long_lived_offer() {
	...
	let offer = alice.node
		.create_offer_builder(Some(BlindedPathType::Full { paths: PATHS_PLACEHOLDER }))
		.unwrap()
		.absolute_expiry(absolute_expiry)
		.build().unwrap();
	assert_eq!(offer.absolute_expiry(), Some(absolute_expiry));
...
fn creates_short_lived_refund() {
	...
	let refund = bob.node
		.create_refund_builder(10_000_000, absolute_expiry, payment_id, Some(BlindedPathType::Compact { paths: PATHS_PLACEHOLDER }), Retry::Attempts(0), None)
		.unwrap()
		.build().unwrap();
...
fn creates_long_lived_refund() {
	...
	let payment_id = PaymentId([1; 32]);
	let refund = bob.node
		.create_refund_builder(10_000_000, absolute_expiry, payment_id, Some(BlindedPathType::Full { paths: PATHS_PLACEHOLDER }), Retry::Attempts(0), None)
		.unwrap()
		.build().unwrap();
	assert_eq!(refund.absolute_expiry(), Some(absolute_expiry));
	assert!(!refund.paths().is_empty());
...
fn fails_creating_or_paying_for_offer_without_connected_peers() {
	...
	match alice.node.create_offer_builder(Some(BlindedPathType::Full { paths: PATHS_PLACEHOLDER })) {
...
fn fails_creating_refund_or_sending_invoice_without_connected_peers() {
	...
	*node_cfgs[1].override_init_features.borrow_mut() = Some(features);
	...
	match david.node.create_refund_builder(
		10_000_000, absolute_expiry, payment_id, Some(BlindedPathType::Full { paths: PATHS_PLACEHOLDER }), Retry::Attempts(0), None
	) {
		Ok(_) => panic!("Expected error"),
		Err(e) => assert_eq!(e, Bolt12SemanticError::MissingPaths),
	}
...
fn fails_creating_refund_with_duplicate_payment_id() {
	...
	assert!(
		nodes[0].node.create_refund_builder(
			10_000, absolute_expiry, payment_id, Some(BlindedPathType::Full { paths: PATHS_PLACEHOLDER }), Retry::Attempts(0), None
		).is_ok()
	);
