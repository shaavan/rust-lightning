...
impl CustomOnionMessageHandler for TestCustomMessageHandler {
	...
	#[cfg(test)]
	fn test_handle_custom_message(&self, msg: Self::CustomMessage, responder: Option<Responder>, add_reply_path: bool) -> ResponseInstruction<Self::CustomMessage> {
		match self.expected_messages.lock().unwrap().pop_front() {
			Some(expected_msg) => assert_eq!(expected_msg, msg),
			None => panic!("Unexpected message: {:?}", msg),
		}
...
fn async_response_over_one_blinded_hop() {
	...
	let response_instruction = nodes[0].custom_message_handler.test_handle_custom_message(message, responder, false);
...
fn do_test_async_response_with_reply_path_over_one_blinded_hop(reply_path_succeed: bool) {
	...
	// 5. Expect Alice to receive the message and create a response instruction for it.
	alice.custom_message_handler.expect_message(message.clone());
	let response_instruction = alice.custom_message_handler.test_handle_custom_message(message, responder, true);
