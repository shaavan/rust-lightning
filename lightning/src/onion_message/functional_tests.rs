...
impl CustomOnionMessageHandler for TestCustomMessageHandler {
	...
	fn handle_custom_message(&self, msg: Self::CustomMessage, context: Vec<u8>, responder: Option<Responder>) -> ResponseInstruction<Self::CustomMessage> {
		...
		match responder {
			Some(responder) if expectation.include_reply_path => {
				responder.respond_with_reply_path(response, MessageContext::Custom(context))
			},
