...
fn one_blinded_hop() {
	...
	let blinded_path =
		BlindedMessagePath::new(&[], nodes[1].node_id, receive_key, context, entropy, &secp_ctx).unwrap();
...
fn two_unblinded_two_blinded() {
	...
	let blinded_path =
		BlindedMessagePath::new(&intermediate_nodes, nodes[4].node_id, receive_key, context, entropy, &secp_ctx)
			.unwrap();
	let path = OnionMessagePath {
...
fn three_blinded_hops() {
    ...
    let blinded_path = BlindedMessagePath::new(&intermediate_nodes, nodes[3].node_id, receive_key, context, entropy, &secp_ctx)
			.unwrap();
	let destination = Destination::BlindedPath(blinded_path);
	let instructions = MessageSendInstructions::WithoutReplyPath { destination };
...
fn async_response_with_reply_path_fails() {
	...
	let context = MessageContext::Custom(Vec::new());
	let entropy = &*bob.entropy_source;
	let receive_key = entropy.get_receive_auth_key();
	let reply_path =
		BlindedMessagePath::new(&[], bob.node_id, receive_key, context, entropy, &secp_ctx)
			.unwrap();
...
fn we_are_intro_node() {
    ...
    let blinded_path = BlindedMessagePath::new(&intermediate_nodes, nodes[2].node_id, receive_key, context, entropy, &secp_ctx)
			.unwrap();
	let destination = Destination::BlindedPath(blinded_path);
	let instructions = MessageSendInstructions::WithoutReplyPath { destination };
	...
	let entropy = &*nodes[1].entropy_source;
	let receive_key = nodes[1].messenger.node_signer.get_receive_auth_key();
    let blinded_path = BlindedMessagePath::new(&intermediate_nodes, nodes[1].node_id, receive_key, context, entropy, &secp_ctx)
			.unwrap();
	let destination = Destination::BlindedPath(blinded_path);
	let instructions = MessageSendInstructions::WithoutReplyPath { destination };
...
fn invalid_blinded_path_error() {
	...
	let mut blinded_path =
		BlindedMessagePath::new(&intermediate_nodes, nodes[2].node_id, receive_key, context, entropy, &secp_ctx)
			.unwrap();
...
fn reply_path() {
	...
	let reply_path =
		BlindedMessagePath::new(&intermediate_nodes, nodes[0].node_id, receive_key, context, entropy, &secp_ctx)
			.unwrap();
	nodes[0]
		.messenger
