...
fn two_unblinded_two_blinded() {
	...
	let blinded_path =
		BlindedMessagePath::new(&intermediate_nodes, nodes[4].node_id, recipient_tlvs, entropy, &secp_ctx)
			.unwrap();
	let path = OnionMessagePath {
...
fn three_blinded_hops() {
	...
	let blinded_path =
		BlindedMessagePath::new(&intermediate_nodes, nodes[3].node_id, recipient_tlvs, entropy, &secp_ctx)
			.unwrap();
	let destination = Destination::BlindedPath(blinded_path);
	let instructions = MessageSendInstructions::WithoutReplyPath { destination };
...
fn we_are_intro_node() {
	...
	let blinded_path =
		BlindedMessagePath::new(&intermediate_nodes, nodes[2].node_id, recipient_tlvs, entropy, &secp_ctx)
			.unwrap();
	let destination = Destination::BlindedPath(blinded_path);
	let instructions = MessageSendInstructions::WithoutReplyPath { destination };
	...
	let entropy = &*nodes[1].entropy_source;
	let blinded_path =
		BlindedMessagePath::new(&intermediate_nodes, nodes[1].node_id, recipient_tlvs, entropy, &secp_ctx)
			.unwrap();
	let destination = Destination::BlindedPath(blinded_path);
	let instructions = MessageSendInstructions::WithoutReplyPath { destination };
...
fn reply_path() {
	...
	let reply_path =
		BlindedMessagePath::new(&intermediate_nodes, nodes[0].node_id, recipient_tlvs, entropy, &secp_ctx)
			.unwrap();
	nodes[0]
		.messenger
