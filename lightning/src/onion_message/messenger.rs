...
/// ];
/// let recipient_data = Some(RecipientData::CustomContext(Vec::new()));
...
pub trait MessageRouter {
	...
	>(
		...
		let peers = peers
			...
			.collect();
		self.create_blinded_paths(recipient, recipient_data, peers, secp_ctx)
	}
}
	...
	ES::Target: EntropySource,
{
	...
	fn create_blinded_paths_from_iter<
		...
		T: secp256k1::Signing + secp256k1::Verification
	>(
		&self, recipient: PublicKey, recipient_data: Option<RecipientData>, peers: I,
		...
		let paths = peer_info.into_iter()
			.map(|(peer, _, _)| {
				BlindedPath::new_for_message(&[peer], recipient, recipient_data.clone(), &*self.entropy_source, secp_ctx)
			})
			.take(MAX_PATHS)
	...
	ES::Target: EntropySource,
{
	...
	>(
		...
		self.create_blinded_paths_from_iter(recipient, recipient_data, peers, secp_ctx, false)
	}
	...
	>(
		...
		self.create_blinded_paths_from_iter(recipient, recipient_data, peers.into_iter(), secp_ctx, true)
	}
}
