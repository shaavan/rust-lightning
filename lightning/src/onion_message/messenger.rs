...
pub enum PeeledOnion<T: OnionMessageContents> {
	...
	Receive(ParsedOnionMessageContents<T>, Option<[u8; 32]>, Option<RecipientData>, Option<BlindedPath>)
}
	...
	CMH::Target: CustomOnionMessageHandler,
{
	...
	match onion_utils::decode_next_untagged_hop(
		...
		Ok((Payload::Receive::<ParsedOnionMessageContents<<<CMH as Deref>::Target as CustomOnionMessageHandler>::CustomMessage>> {
			...
			match (&message, receive_tlvs) {
				(ParsedOnionMessageContents::Offers(_), ReceiveTlvs::OffersData(recipient_data)) => {
					Ok(PeeledOnion::Receive(message, None, Some(recipient_data), reply_path))
				}
				(ParsedOnionMessageContents::Custom(_), ReceiveTlvs::CustomData(data)) => {
					Ok(PeeledOnion::Receive(message, data, None, reply_path))
				}
				_ => Err(()),
			}
			...
			Ok(PeeledOnion::Receive(message, path_id, recipient_data, reply_path)) => {
				...
				match message {
					ParsedOnionMessageContents::Offers(msg) => {
						...
						let response_instructions = self.offers_handler.handle_message(msg, responder, recipient_data.unwrap());
					...
					ParsedOnionMessageContents::Custom(msg) => {
						...
						let response_instructions = self.custom_handler.handle_custom_message(msg, responder, RecipientData::new());
						let _ = self.handle_onion_message_response(response_instructions);
					},
				}
			},
...
fn packet_payloads_and_keys<T: OnionMessageContents, S: secp256k1::Signing + secp256k1::Verification>(
	...
	if let Some(control_tlvs) = final_control_tlvs {
		...
		payloads.push((Payload::Receive {
			control_tlvs: ReceiveControlTlvs::Unblinded(ReceiveTlvs::new_offers_data(RecipientData::new())),
			reply_path: reply_path.take(),
			message,
		}, prev_control_tlvs_ss.unwrap()));
	}
