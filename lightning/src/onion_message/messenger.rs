	...
	CMH::Target: CustomOnionMessageHandler,
{
	...
	pub fn handle_onion_message_response<T: OnionMessageContents>(
		...
		let (response, context, create_reply_path) = match response {
			ResponseInstruction::WithReplyPath(response, context) => (response, Some(context), true),
			ResponseInstruction::WithoutReplyPath(response) => (response, None, false),
			ResponseInstruction::NoResponse => return Ok(None),
		};

		let message_type = response.message.msg_type();
		let reply_path = if create_reply_path {
			match self.create_blinded_path(context.unwrap()) {
