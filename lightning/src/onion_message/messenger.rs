...
pub trait CustomOnionMessageHandler {
	...
	fn handle_custom_message(&self, message: Self::CustomMessage, responder: Option<Responder>, recipient_data: Vec<u8>) -> ResponseInstruction<Self::CustomMessage>;
				...
				match message {
					ParsedOnionMessageContents::Offers(msg) => {
						...
						let offers_data = match recipient_data {
							RecipientData::Offers(data) => data,
							RecipientData::Custom(_) => {
								log_error!(logger, "Received Custom Recipient Data for Offers Message.");
								// Temporarily creating an empty OffersData here, till figuring out a better
								// handling mechanism.
								OffersData {
									payment_id: None
								}
					...
					ParsedOnionMessageContents::Custom(msg) => {
						...
						let recipient_data = match recipient_data {
							RecipientData::Offers(_) => {
								log_trace!(logger, "Received Offers Recipient Data for Custom Message.");
								Vec::new()
							}
							RecipientData::Custom(data) => data,
						};
...
fn packet_payloads_and_keys<T: OnionMessageContents, S: secp256k1::Signing + secp256k1::Verification>(
	...
	if let Some(control_tlvs) = final_control_tlvs {
		...
		payloads.push((Payload::Receive {
			control_tlvs: ReceiveControlTlvs::Unblinded(ReceiveTlvs { path_id: None, recipient_data: RecipientData::custom(Vec::new()) }),
			reply_path: reply_path.take(),
			message,
		}, prev_control_tlvs_ss.unwrap()));
	}
