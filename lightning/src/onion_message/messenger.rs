	...
	CMH::Target: CustomOnionMessageHandler,
{
	...
	let compute_shared_secrets = |blinding_point: &PublicKey, packet_pubkey: &PublicKey| -> Result<(SharedSecret, [u8; 32]), ()> {
	...
	let process_receive_payload = |message, context, reply_path| -> Result<PeeledOnion<<<CMH>::Target as CustomOnionMessageHandler>::CustomMessage>, ()> {
	...
	let compute_onion_message = |packet_pubkey: PublicKey,
	 ...
	 -> Result<OnionMessage, ()> {
		let new_pubkey =
			match onion_utils::next_hop_pubkey(&secp_ctx, packet_pubkey, &current_onion_decode_ss) {
				Ok(pk) => pk,
				Err(e) => {
	...
	let (control_tlvs_ss, onion_decode_ss) = compute_shared_secrets(&msg.blinding_point, &msg.onion_routing_packet.public_key)?;
	...
	match next_hop {
			...
			loop {
				...
				match current_next_hop {
					Ok((
						...
						let new_pubkey =
							match onion_utils::next_hop_pubkey(&secp_ctx, current_message.onion_routing_packet.public_key, &current_onion_decode_ss) {
								Ok(pk) => pk,
								Err(e) => {
									log_trace!(logger, "Failed to compute next hop packet pubkey: {}", e);
									return Err(());
						...
						current_message = OnionMessage { blinding_point, onion_routing_packet: outgoing_packet };
					},
					Ok((
						Payload::Receive {
							message,
					...
					Err(e) => {
						log_trace!(logger, "Errored decoding onion message packet in dummy chain: {:?}", e);
						return Err(());
					},
					_ => {
		...
		Ok((
			...
			log_trace!(logger, "Received Subsequent dummy without preceding Primary dummy - invalid structure");
			Err(())
		},
		Ok((
