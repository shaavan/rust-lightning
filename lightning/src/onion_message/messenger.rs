	...
	ES::Target: EntropySource,
{
	...
	>(
		...
		let paths = peer_info
			.into_iter()
			.map(|(peer, _, _)| {
				BlindedMessagePath::new(&[peer], recipient, recipient_tlvs.clone(), entropy, secp_ctx)
			})
			.take(MAX_PATHS)
	...
	pub(crate) fn create_blinded_paths<T: secp256k1::Signing + secp256k1::Verification>(
		network_graph: &G, recipient: PublicKey, recipient_tlvs: ReceiveTlvs, peers: Vec<PublicKey>,
