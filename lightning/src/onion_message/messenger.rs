	...
	CMH::Target: CustomOnionMessageHandler,
{
	...
	fn handle_onion_message_response<T: OnionMessageContents>(
		&self, response: ResponseInstruction<T>, reply_path: BlindedPath, log_suffix: fmt::Arguments
	) {
			...
			Ok(PeeledOnion::Receive(message, path_id, reply_path)) => {
				...
				match message {
					ParsedOnionMessageContents::Offers(msg) => {
						let received_message = ReceivedOnionMessage::new(msg, reply_path.clone(), path_id.clone());
						let response_instructions = self.offers_handler.handle_message(received_message);
						self.handle_onion_message_response(response_instructions, reply_path.unwrap(), format_args!(
					...
					ParsedOnionMessageContents::Custom(msg) => {
						let message = ReceivedOnionMessage::new(msg, reply_path.clone(), path_id.clone());
