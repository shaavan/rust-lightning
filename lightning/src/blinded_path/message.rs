...
impl Writeable for PrimaryDummyTlv {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
		encode_tlv_stream!(writer, {
			// Unauthenticated dummy TLV are blank placeholders, so we don't write them.
	...
	ES::Target: EntropySource,
{
	...
	let primary_dummy = {
		...
		ControlTlvs::Dummy(DummyTlv::Primary(PrimaryDummyTlv {
			dummy_tlv,
			authentication: (hmac, nonce),
		}))
	};

	let subsequent_dummies = (0..dummy_hops_count.saturating_sub(1)).map(|_| {
		ControlTlvs::Dummy(DummyTlv::Subsequent)
	});

	let receive = std::iter::once(ControlTlvs::Receive(ReceiveTlvs {
		context: Some(context),
	}));

	let tlvs = forward_tlvs
		.chain(std::iter::once(primary_dummy))
		.chain(subsequent_dummies)
		.chain(receive);
